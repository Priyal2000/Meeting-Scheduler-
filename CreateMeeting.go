...
//isParticipantBusy : Checks if the participant is not invited or busy with other meetings
func isParticipantBusy(thismeet Meeting) error {
		...
		if thisperson.Rsvp == "Yes" {
			...
			for cursor.Next(ctx) {
				...
				if (thismeet.Starttime >= meet.Starttime && thismeet.Starttime <= meet.Endtime) ||
					...
					returnerror := "Error 400: Participant " + thisperson.Name + " Meeting Clash"
